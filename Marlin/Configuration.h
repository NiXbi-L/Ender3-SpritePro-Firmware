...
#define BED_MAXTEMP      125  // Ender Configs
 ...
 */
#define DEFAULT_AXIS_STEPS_PER_UNIT   { 80, 80, 400, 424.9 }  // X Y Z E - Sprite Pro direct drive
...
//#define USE_PROBE_FOR_Z_HOMING  // Отключено: используем механический концевик Z
 ...
 */
//#define BLTOUCH  // Отключено: базовый Ender 3 без зонда, только концевики
...
//#define Z_CLEARANCE_FOR_HOMING  10   // (mm) Minimal Z height before homing (G28) for Z clearance above the bed  // MRiscoC
...
//#define AUTO_BED_LEVELING_UBL  // Требует зонд
#define MESH_BED_LEVELING  // Ручная сетка без зонда (регулировка соплом)
...
#define LCD_BED_TRAMMING  // ProUI has a bed tramming menu
  ...
  //#define BED_TRAMMING_USE_PROBE  // Отключено: без зонда, выравнивание соплом
 ...
 * Отключено: без зонда Z домается концевиком на текущей XY позиции
 */
//#define Z_SAFE_HOMING

#if ENABLED(Z_SAFE_HOMING)
  #define Z_SAFE_HOMING_X_POINT 127.5
  #define Z_SAFE_HOMING_Y_POINT 130
#endif
...
#define NOZZLE_PARK_FEATURE  // MRiscoC Enabled
  ...
  // Specify a park position as { X, Y, Z_raise } — вперёд к оператору, но в пределах лимитов
